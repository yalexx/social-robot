...
export module Robot {
    ...
    export function doScroll(scrollTime, callback?) {
    ...
    export function clickUploadPicture() {
        setTimeout(() => {
    ...
    export function typeInputData(io, data, emailInputPosition, callback, app): void {
        setTimeout(() => {
    ...
    export function typeRobotEmail(io, data, pos, callback, app): void {
    ...
    export function typeRobotEmailUser(io, data, pos, callback, app): void {
        let emailArray = data.split('@');
        let emailName = emailArray[0];
        setTimeout(() => {
            clickElement(pos, 1, app);
    ...
    export function typeRobotEmailName(io, data, pos, callback, appRef): void {
    ...
    export function moveMouse(pos, appRef): void {
    ...
    function limitInsideDevice(pos) {
        ...
        if (pos.top > maxBottom) pos.top = maxBottom;
        if (pos.top < topOffset) pos.top = topOffset;
        if (pos.left > maxRight) pos.left = maxRight;

        return pos;

    }

    export function clickElement(elementInfo, count, appRef): void {

        if (elementInfo == 'not_found' &&  elementInfo.top !== 0 && elementInfo.left !== 0 || elementInfo.width === 0 && elementInfo.height === 0) console.log('Element not found. ' + elementInfo);
        else {
    ...
    export function runActionListener(socket, appRef): void {
