...
function rebindEvents() {
    $('[data-Action="RemoveUserAgent"]').unbind("click"), $('[data-Action="RemoveUserAgent"]').click(function () {
        var t = $(this), n;
        if (confirm("Remove this User-Agent?")) for (n = 0; n < userAgents.length; n++) userAgents[n].UserAgents.forEach(function (i, r) {
        ...
        var t, n;
        if ($("#UserAgentId").val(""), $("#UserAgentName").val(""), $("#UserAgentString").val(""), t = $(this), t.attr("data-Id") != "") for (n = 0; n < userAgents.length; n++) userAgents[n].UserAgents.forEach(function (n) {
...
$(function () {
    loadStorageData(), loadSettingsData(), loadUserAgentGroups(), loadUserAgents(), loadSettings(), $("#UserAgentGroup").change(function () {
        ...
        var i = $("#UserAgentName").val(), r = $("#UserAgentString").val(), u, f, n, t;
        if (i = i.replace(/(?:(?:^|\n)\s+|\s+(?:$|\n))/g, "").replace(/\s+/g, " "), r = r.replace(/(?:(?:^|\n)\s+|\s+(?:$|\n))/g, "").replace(/\s+/g, " "), i != "" && r != "") {
            for (u = "Id_" + Math.floor(Math.random() * 1e6 + 1), f = {
                Id: u,
                Name: i,
                UserAgent: r
            }, n = 0; n < userAgents.length; n++) if (userAgents[n].Id == $("#UserAgentGroup").val()) if ($("#UserAgentId").val() != "") for (t = 0; t < userAgents[n].UserAgents.length; t++) $("#UserAgentId").val() == userAgents[n].UserAgents[t].Id && (userAgents[n].UserAgents[t].Name = i, userAgents[n].UserAgents[t].UserAgent = r); else userAgents[n].UserAgents.push(f);
